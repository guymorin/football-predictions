<?php
use FootballPredictions\Theme;

$array = array(
    "account" => "Account",
    "addTeamTo" => "Add team to ",
    "added" => "Added",
    "administration" => "administration",
    "administrator" => "administrateur",
    "away" => "Away",
    "bestPlayers" => "Best players",
    "bestPlayersByTeam" => "Best players by team",
    "bet" => "Bet",
    "careful" => "Careful&nbsp,game!",
    "carefulToo" => "Too&nbsp,careful&nbsp,game!",
    "championship" => "Championship",
    "contributor" => "contributor",
    "create" => Theme::icon('create')." Create",
    "created" => "Created",
    "createAChampionship" => "Create a championship",
    "createAMatch" => "Create a match",
    "createAMatchday" => "Create a matchday",
    "createAnAccount" => "Create an account",
    "createAnAdmin" => "Create an administrator account",
    "createAPlayer" => "Create a player",
    "createASeason" => "Create a season",
    "createATeam" => "Create a team",
    "createDatabase" => "Create the database",
    "createTheMatchdays" => "Create the matchdays",
    "criterionSum" => "Criterion sum",
    "currentForm" => "Current form",
    "dashboard" => "Dashboard",
    "date" => "Date",
    "defender" => "Defender",
    "delete" => Theme::icon('delete')." Delete",
    "deleted" => "Deleted",
    "draw" => "D",
    "earning" => "Earning",
    "earningByBet" => "Earning&nbsp,by&nbsp,bet",
    "emailAddress" => "E-mail address",
    "error" => "Error",
    "errorAlnum" => "alphanumeric characters only",
    "errorConnection" => "problem with the database connection",
    "errorDate" => "invalid date",
    "errorDigit" => "integer numeric characters only",
    "errorExists" => "this name already exists",
    "errorExport" => "problem with the export",
    "errorFields" => "some fields have not been filled in correctly",
    "errorImport" => "problem with the import",
    "errorNotField" => "field not filled",
    "errorNum" => "numeric characters only",
    "errorPassword" => "invalid username or password",
    "errorPasswordNoMatch" => "those passwords didn't match",
    "errorPath" => "invalid path",
    "errorPosition" => "invalid position",
    "errorResult" => "invalid result",
    "errorSeasonName" => "9 characters max.",
    "errorWritable" => "Writable access needed on the installation repository",
    "exit" => "Exit",
    "exited" => "Exited",
    "firstname" => "Firstname",
    "forward" => "Forward",
    "general" => "General",
    "goalkeeper" => "Goalkeeper",
    "home" => "Home",
    "homepage" => "Homepage",
    "install" => "Install",
    "installHost" => "Host",
    "installName" => "Name",
    "installUser" => "User",
    "installPass" => "Password",
    "language" => "Language",
    "listAccounts" => "List of accounts",
    "lastSave" => "Last backup",
    "listChampionships" => "List of championship",
    "listMatchdays" => "List of matchdays",
    "login" => "Username",
    "logon" => "Log on",
    "logoff" => "Log off",
    "lose" => "L",
    "marketValue" => "Marketvalue",
    "matchday" => "Matchday",
    "matchdayNext" => "Next matchday",
    "matchdays" => "Matchdays",
    "matchgame" => "Match",
    "matchNumber" => "Number of matchgame",
    "matchPlayed" => "Played matchgames",
    "MD" => "MD",
    "midfielder" => "Midfielder",
    "modify" => Theme::icon('modify')." Modify",
    "modified" => "Modified",
    "modifyAChampionship" => "Modify a championship",
    "modifyAMatch" => "Modify a match",
    "modifyAnAccount" => "Modify an account",
    "modifyAPlayer" => "Modify a player",
    "modifyPredictions" => "Modify prediction²s",
    "modifyASeason" => "Modify a season",
    "modifyATeam" => "Modify a team",
    "modifyAMarketValue" => "Modify a marketvalue",
    "modifyAMatchday" => "Modify a matchday",
    "motivation" => "Motivation",
    "myAccount" => "My account",
    "name" => "Name",
    "no" => "No",
    "notPlayed" => "Not played",
    "noChampionship" => "No championship",
    "noMatch" => "No match",
    "noMatchday" => "No matchday",
    "noSeason" => "No season",
    "noTeam" => "No team",
    "number" => "Number",
    "matchdayNumber" => "Number of matchdays",
    "odds" => "Odds",
    "oddsAveragePlayed" => "Average<br />played<br />odds",
    "password" => "Password",
    "passwordConfirm" => "Confirm password",
    "physicalForm" => "Physical form",
    "player" => "Player",
    "position" => "Position",
    "prediction" => "Prediction",
    "predictions" => "Predictions",
    "predictionsHistory" => "Predictions history",
    "profit" => "Profit",
    "profitByMatchday" => "Profit by matchday",
    "profitEvolution" => "Profit evolution",
    "pts" => "Pts",
    "quickNav" => "Quick nav",
    "rating" => "Rating",
    "ratingAverage" => "Average rating",
    "redCards" => "Red cards",
    "result" => "Result",
    "results" => "Results",
    "ROI" => "ROI",
    "ROIisLosing" => "ROI&nbsp,is&nbsp,losing!",
    "ROIisNeutral" => "ROI&nbsp,is&nbsp,neutral!",
    "ROIwins" => "ROI&nbsp,wins!",
    "ROIisExcellent" => "ROI&nbsp,is&nbsp,excellent!",
    "role" => "Role",
    "save" => "Save",
    "saved" => "Saved",
    "season" => "Season",
    "select" => "Select",
    "selectAnotherSeason" => "Select another season",
    "selected" => "Selected",
    "selectTheChampionship" => "Select a championship",
    "selectTheMatchday" => "Select a matchday",
    "selectThePlayer" => "Select a player",
    "selectTheSeason" => "Select a season",
    "selectTheTeams" => "Select the teams",
    "speculative" => "Speculative&nbsp,game!",
    "speculativeToo" => "Too&nbsp,speculative&nbsp,game!",
    "statistics" => "Statistics",
    "standing" => "Standing",
    "success" => "Success",
    "successRate" => "Success rate",
    "swithToAuto" => "Switch to semi-automatical",
    "swithToManual" => "Switch to manual mode",
    "team" => "Team",
    "teams" => "Teams",
    "teamOfTheWeek" => "Team of the week",
    "theme" => "Theme",
    "site" => "FP",
    "siteSubTitle" => "Football predictions",
    "siteData" => "Website datas",
    "yes" => "Yes",
    "warningExists" => 'Warning: this name already exists!',
    "weather" => "Weather",
    "weathercode" => "Weather code",
    "win" => "G"
);
?>
